  ...
  try {
    ...
    const chargeRows = (await sql`
      select consume_markai_credits(
        ...
        ${JSON.stringify({ platform, objective, variations })}::jsonb
      ) as result
    ...
    if (charged > 0 && modelKey) {
      try {
        await sql`select refund_markai_credits(${session.workspaceId}::uuid, ${session.userId}::uuid, ${modelKey}, ${charged}, 'ads_generation_failed')`;
      } catch (refundError) {
